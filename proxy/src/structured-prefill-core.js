...
const STRUCTURED_PREFILL_UNICODE_ESCAPE_REPLACEMENTS = Object.freeze({
    '00a0': ' ',
    '00ad': '',
    '2010': '-',
    '2011': '-',
...
function repairVisibleUnicodeEscapeArtifacts(text) {
    return String(text ?? '').replace(
        /\\*u(00a0|00ad|2010|2011|2012|2013|2014|2015|2018|2019|201a|201b|201c|201d|201e|201f|2026|2212)/gi,
        (match, code) => {
            const replacement = STRUCTURED_PREFILL_UNICODE_ESCAPE_REPLACEMENTS[String(code ?? '').toLowerCase()];
...
function normalizeStructuredPrefillUnicodeArtifacts(text) {
    return repairVisibleUnicodeEscapeArtifacts(replaceCommonUnicodePunctuationWithAscii(text));
}
...
function decodeStructuredText(text, context) {
    if (context.schemaMode === 'enum_prefix' || context.schemaMode === 'enum_exact') {
        return normalizeStructuredPrefillUnicodeArtifacts(String(text ?? ''));
    }
    return normalizeStructuredPrefillUnicodeArtifacts(straightenCurlyQuotes(decodeNewlines(text, context.newlineToken)));
}
